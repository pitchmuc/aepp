...
class ClassManager:
    ...
    def searchField(self,string:str,partialMatch:bool=True,caseSensitive:bool=False)->list:
        ...
        """
        definition = self.aepclass.get('definitions',self.aepclass.get('properties',{}))
        data = __searchAlgo__(self.aepclass.get('allOf',[]),definition,string,partialMatch,caseSensitive)
        return data
    ...
    def to_dict(self,typed:bool=True,save:bool=False)->dict:
        ...
        if self.EDITABLE:
            behaviorDefinition = self.behaviorDefinition.get('properties')
            dataBehavior = __transformationDict__(behaviorDefinition,typed,dictionary=data)
            data = __simpleDeepMerge__(data,dataBehavior)
        if save:
